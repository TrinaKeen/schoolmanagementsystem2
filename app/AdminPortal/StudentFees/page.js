...
export default function StudentFeesPage() {
  ...
  const handleViewClick = async (studentNumber) => {
    try {
      const res = await fetch(`/api/admin/fetchStudentFeeBreakdown?student_number=${studentNumber}`);
      const data = await res.json();
      setSelectedFee(data);
      setShowModal(true);
    } catch (err) {
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {sortedFees.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="5" className={styles.noData}>No matching student fees found.</td>
                  </tr>
                )}
              </tbody>
            </table>
  ...
  <div className={styles.modalBackdrop}>
    <div className={styles.modalContent}>
      ...
      <div className={styles.modalSection}>
        <p><strong>Program ID:</strong> {selectedFee.program_id}</p>
        <p><strong>Payment Status:</strong> {selectedFee.payment_status}</p>
      </div>
