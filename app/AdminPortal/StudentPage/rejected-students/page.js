...
const RejectStudentPage = () => {
  ...
  const fetchStudents = async () => {
    try {
      const res = await fetch("/api/admin/rejected-students");
      if (!res.ok) {
  ...
  const handleRejectStudent = async (student_number) => {
    try {
      const response = await fetch("/api/admin/update-approval", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
              ...
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={student.student_number}>
                    ...
                    <td>{student.approval_status || "Not Active"}</td>
                    <td>
                      <button
...
export default RejectStudentPage;
